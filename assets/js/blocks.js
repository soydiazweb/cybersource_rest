(function () {
	...
	function Content(props) {
		var cardNumber = useState(''), expiry = useState(''), cvv = useState(''), status = useState('');
		...
		}, [onPaymentProcessing, props.emitResponse.responseTypes.SUCCESS, props.emitResponse.responseTypes.ERROR, props.billing, cardNumber[0], expiry[0], cvv[0]]);
		...
		return el('div', {className:'wc-cybs-block-fields'},
			...
			el('div', {style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}},
				...
				el('label', null, 'CVV', el('input', {type:'password', className:'cybs-card-cvv', inputMode:'numeric', autoComplete:'cc-csc', maxLength:4, placeholder:'\u2022\u2022\u2022\u2022', value:cvv[0], onChange:function(e){cvv[1](e.target.value.replace(/\D/g, '').slice(0, 4));}}))
			),
			el('div', {role:'status', 'aria-live':'polite'}, status[0])
		);
	}

	var labelText = decode(settings.title || 'Tarjeta de credito o debito');
